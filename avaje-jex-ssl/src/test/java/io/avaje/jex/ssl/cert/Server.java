package io.avaje.jex.ssl.cert;

import java.io.ByteArrayInputStream;
import java.io.IOException;
import java.io.InputStream;
import java.net.URISyntaxException;
import java.nio.charset.StandardCharsets;
import java.nio.file.Path;
import java.util.function.Supplier;

import io.avaje.jex.spi.ClassResourceLoader;

public final class Server {
  static ClassResourceLoader loader = ClassResourceLoader.fromClass(Server.class);

  public static final String CERTIFICATE_AS_STRING =
      """
  	-----BEGIN CERTIFICATE-----
  	
  	-----END CERTIFICATE-----""";

  public static final Supplier<InputStream> CERTIFICATE_INPUT_STREAM_SUPPLIER =
      string2IS(CERTIFICATE_AS_STRING);

  public static Supplier<InputStream> string2IS(String cert) {
    return () -> new ByteArrayInputStream(cert.getBytes(StandardCharsets.UTF_8));
  }

  public static final String NORWAY_CERTIFICATE_AS_STRING =
      """
  	-----BEGIN CERTIFICATE-----
  	
  	-----END CERTIFICATE-----""";
  public static final String NON_ENCRYPTED_KEY_AS_STRING =
      """
  	-----BEGIN PRIVATE KEY-----
  	
  	-----END PRIVATE KEY-----""";
  public static final Supplier<InputStream> NON_ENCRYPTED_KEY_INPUT_STREAM_SUPPLIER =
      string2IS(NON_ENCRYPTED_KEY_AS_STRING);
  public static final String ENCRYPTED_KEY_AS_STRING =
      """
  	-----BEGIN ENCRYPTED PRIVATE KEY-----
  	
  	na5mQXa7ctzrJ+bqW9B+QLBX6KZk3tRnigYO6Fum0t7I
  	-----END ENCRYPTED PRIVATE KEY-----""";
  public static final Supplier<InputStream> ENCRYPTED_KEY_INPUT_STREAM_SUPPLIER =
      string2IS(ENCRYPTED_KEY_AS_STRING);
  public static final String GOOGLE_CERTIFICATE_AS_STRING =
      """
  	-----BEGIN CERTIFICATE-----
  	
  	-----END CERTIFICATE-----""";
  public static final String GOOGLE_KEY_AS_STRING =
      """
  	-----BEGIN PRIVATE KEY-----
  	
  	-----END PRIVATE KEY-----""";
  public static final String CERTIFICATE_FILE_NAME = "test-certs/server/cert.crt";
  public static final String ENCRYPTED_KEY_FILE_NAME = "test-certs/server/encrypted.key";
  public static final String NON_ENCRYPTED_KEY_FILE_NAME = "test-certs/server/passwordless.key";
  public static final String CERTIFICATE_PATH;
  public static final String ENCRYPTED_KEY_PATH;
  public static final String NON_ENCRYPTED_KEY_PATH;
  public static final String KEY_PASSWORD = "password";
  public static final String JKS_KEY_STORE_NAME = "test-certs/server/keystore.jks";
  public static final Supplier<InputStream> JKS_KEY_STORE_INPUT_STREAM_SUPPLIER =
      () -> {
        try {
          return loader.loadResource(JKS_KEY_STORE_NAME).openStream();
        } catch (IOException e) {
          throw new RuntimeException(e);
        }
      };
  public static final String P12_KEY_STORE_NAME = "test-certs/server/keystore.p12";
  public static final Supplier<InputStream> P12_KEY_STORE_INPUT_STREAM_SUPPLIER =
      () -> {
        try {
          return loader.loadResource(P12_KEY_STORE_NAME).openStream();
        } catch (IOException e) {
          throw new RuntimeException(e);
        }
      };
  public static final String NORWAY_JKS_KEY_STORE_NAME = "test-certs/server/norway.jks";
  public static final String NORWAY_P12_KEY_STORE_NAME = "test-certs/server/norway.p12";
  public static final String JKS_KEY_STORE_PATH;
  public static final String P12_KEY_STORE_PATH;
  public static final String NORWAY_JKS_KEY_STORE_PATH;
  public static final String NORWAY_P12_KEY_STORE_PATH;
  public static final String KEY_STORE_PASSWORD = "password";

  static {
    try {

      CERTIFICATE_PATH =
          Path.of(loader.loadResource(CERTIFICATE_FILE_NAME).toURI()).toAbsolutePath().toString();
      ENCRYPTED_KEY_PATH =
          Path.of(loader.loadResource(ENCRYPTED_KEY_FILE_NAME).toURI()).toAbsolutePath().toString();
      NON_ENCRYPTED_KEY_PATH =
          Path.of(loader.loadResource(NON_ENCRYPTED_KEY_FILE_NAME).toURI())
              .toAbsolutePath()
              .toString();
      JKS_KEY_STORE_PATH =
          Path.of(loader.loadResource(JKS_KEY_STORE_NAME).toURI()).toAbsolutePath().toString();
      P12_KEY_STORE_PATH =
          Path.of(loader.loadResource(P12_KEY_STORE_NAME).toURI()).toAbsolutePath().toString();
      NORWAY_JKS_KEY_STORE_PATH =
          Path.of(loader.loadResource(NORWAY_JKS_KEY_STORE_NAME).toURI())
              .toAbsolutePath()
              .toString();
      NORWAY_P12_KEY_STORE_PATH =
          Path.of(loader.loadResource(NORWAY_P12_KEY_STORE_NAME).toURI())
              .toAbsolutePath()
              .toString();
    } catch (URISyntaxException e) {
      throw new RuntimeException(e);
    }
  }
}
