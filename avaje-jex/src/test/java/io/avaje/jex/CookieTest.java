...
class CookieTest {
  ...
  void format() {
    ...
    assertEquals(
        "key=val; Domain=dom", Cookie.of("key", "val").domain("dom").toString());
    assertEquals("key=val; Path=/pt", Cookie.of("key", "val").path("/pt").toString());
    assertEquals(
        "key=val; Max-Age=10",
        Cookie.of("key", "val").maxAge(Duration.ofSeconds(10)).toString());
    ...
    assertEquals(
        "key=val; Partitioned",
        Cookie.of("key", "val").partitioned(true).toString());
    assertEquals(
  ...
  void partitioned() {
    ...
    assertEquals(
        "k=v; Secure; Partitioned",
        Cookie.of("k", "v").secure(true).partitioned(true).toString());
    assertEquals(
        "k=v; Secure",
        Cookie.of("k", "v").secure(true).partitioned(false).toString());
  }
  ...
  void sameSite() {
    ...
    assertEquals(
        "key=val; Secure",
        Cookie.of("key", "val").secure(true).sameSite(null).toString());
  }
  ...
  void format_all() {
    ...
    assertEquals(
        "key=val; Domain=dom; Path=/pt; SameSite=Strict; Secure; HttpOnly; Partitioned", cookie.toString());
  }
}
