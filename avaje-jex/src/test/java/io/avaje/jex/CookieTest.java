...
class CookieTest {
  ...
  void format() {
    ...
    assertEquals(
        "key=val; Max-Age=10", Cookie.of("key", "val").maxAge(Duration.ofSeconds(10)).toString());
    ...
    assertEquals(
        "key=val; SameSite=Strict",
        Cookie.of("key", "val").sameSite(Cookie.SameSite.Strict).toString());
    assertEquals(
        "key=val; Secure; HttpOnly",
        Cookie.of("key", "val").httpOnly(true).secure(true).toString());
  }
  ...
  void partitioned() {
    ...
    assertEquals(
        "k=v; Secure; Partitioned", Cookie.of("k", "v").secure(true).partitioned(true).toString());
  ...
  void sameSite() {
    ...
    assertEquals(
        "key=val; SameSite=Strict; Secure",
        Cookie.of("key", "val").secure(true).sameSite(Cookie.SameSite.Strict).toString());
    assertEquals(
        "key=val; SameSite=Lax; Secure",
        Cookie.of("key", "val").secure(true).sameSite(Cookie.SameSite.Lax).toString());
    assertEquals(
        "key=val; SameSite=None; Secure",
        Cookie.of("key", "val").secure(true).sameSite(Cookie.SameSite.None).toString());
  ...
  void format_all() {
    ...
    assertEquals(
        "key=val; Domain=dom; Path=/pt; SameSite=Strict; Secure; HttpOnly; Partitioned",
        cookie.toString());
  }
}
