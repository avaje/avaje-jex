...
class MultiHandlerTest {
  ...
  void test() {
    ...
    HttpResponse<String> hxRes =
        pair.request().header("Hx-Request", "true").path("hi").GET().asString();
  ...
  void testWithPathParam() {
    ...
    HttpResponse<String> hxRes =
        pair.request().header("Hx-Request", "true").path("hi/42").GET().asString();
    assertThat(hxRes.statusCode()).isEqualTo(200);
    assertThat(hxRes.body()).isEqualTo("HxResponse|42");

    HttpResponse<String> h2Res =
        pair.request().header("H2-Request", "true").path("hi/42").GET().asString();
