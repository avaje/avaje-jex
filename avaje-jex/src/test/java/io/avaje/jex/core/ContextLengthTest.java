...
class ContextLengthTest {
  ...
  static TestPair init() {
    var app = Jex.create().routing(routing -> routing.post(
            "/", ctx -> ctx.text("contentLength:" + ctx.contentLength() + " type:" + ctx.contentType()))
  ...
  void fullUrl_no_queryString() {
    HttpResponse<String> res =
        pair.request().path("fullUrl").path("noQuery").GET().asString();
